...
arrasto aerodinâmico, $F_g$ é a componente do peso que esta direção da velocidade e $F_{r}$ é a resistência ao rolamentos dos pneus no pista.
...
A partir das Equações (\ref{eq:MotorCC}) e (\ref{eq:Transmissao}), desconsiderando as perdas por atrido visco nos rolamentos, as perdas causada pela resistência interna na bateira e
a perda no conversor de potencia, o conjunto que descreve o a força do sistema de propulsão $F_t$ de um veiculo elétrico é
...
A soluçai de um problema de controle ótimo é uma sequencia de sinais de controle em malha aberta e um conjunto de paramentos, o comportamento esperado é
exemplificado na Figura \ref{fig:simpleOPC}.

De forma geral, há três abordagens básicas para solução computacional de um OPC, (a) programação dinâmica, (b) métodos indiretos e (c) métodos diretos, 
...
Nas abordagens sequenciais, os estado $x(t)$ são uma função implícita dos controles $u(t)$ e do valor inicial $x_0$. Dessa forma, a simulação
e as iterações de otimização procedem sequencialmente e o NLP possui apenas o controle discretizado como graus de liberdade de otimização. O método mais usado de abordagem sequencial é o \textit{Direct Single Shooting}.
...
Ela segue o paradigma de programação orientada a objetos, um resumo de suass principais classes de objetos é apresentado na Figura \ref{fig:objetosFalcon}. 
A classe \lstinline[style=Matlab-editor]{Problem} é a principal, nela são configurados as fases (\lstinline[style=Matlab-editor]{Phases}) do OPC, os parâmetros otimizáveis, o custo de Mayer e
...
O otimizador utilizado neste trabalho é o IPOPT, configuração pré-definida da biblioteca FALCON.m. Mais informações sobre a utilização do FALCON.m pode ser constuldas em seu manual \citeauthor{manual:Falcon}.
\begin{figure}[h]
	\centering
	...
	\end{tikzpicture}
	\caption*{\footnotesize Fonte: Adaptado de \citeauthor{phd:Matthias}}
\end{figure}
...
O projeto COIN-OR, do inglês \textit{COmputational INfrastructure for Operations Research},
 ...
 método de pontos interiores com filtro apresentado por \citeauthor{article:Ipopt}. Sua licença é a EPL que permite uso gratuito e acesso ao codigo. Mais detalhes sobre esta biblioteca podem ser obtidos na pagina do seu repositório no GitHub\textsuperscript{\textregistered} \url{https://github.com/coin-or/Ipopt}.
