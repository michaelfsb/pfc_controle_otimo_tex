...
\begin{figure}[h]
    ...
    \input{Metodologia/Figuras/altitude_pista2.tex}
    \label{fig:altitude_pista}
    \caption*{\footnotesize{Fonte: Dados da equipe de competição \textit{Duke Electric Vehicles}.}}
\end{figure}
...
em que os coeficientes $a_n$, $b_n$ e $c_n$ estão apresentados na Tabela \ref{tab:coeficientes}.
...
\begin{mini!}
	{i(t), \varphi, T}{\int_{0}^{T} i(t) \cdot \left(i(t) \cdot R_a + \frac{Kv \cdot v(t) \cdot \varphi}{r_r} \right) \,\mathrm{d}t \label{eq:ObjOCP_final}}
	...
	\addConstraint{i(t)-\frac{u_{bat}-Kv\cdot(v(t) \cdot \varphi/r_r)}{R_a}}{\leq 0, \quad}{t \in \left[0,T\right]  \label{eq:C5_OCP_final}}
    \addConstraint{0 \leq i(t)}{\leq i_{max}, \quad}{t \in \left[0,T\right]  \label{eq:C6_OCP_final}}
    \addConstraint{0 \leq v(t)}{\leq v_{max}, \quad}{t \in \left[0,T\right]  \label{eq:C7_OCP_final}}
